...
public class GoogleIAP : MonoBehaviour, IDetailedStoreListener
{
    public enum PurchaseState
    {
        INITIALIZE_FAILED,
        NOT_REGISTERED,
        ALREADY_PURCHASE_STATE,
        SUCCESS
    }
    ...
    void Init()
    {
        ...
        List<ProductData> prData = ProductManager.GetAllProduct();
    ...
    {
        Debug.Log($"OnInitializeFailed failed : {error} message : {message}");
    }
    ...
    {
        Debug.Log($"OnInitializeFailed failed : {error}");
    }
    ...
    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs args)
    {
        ...
        {
            Debug.Log("네트워크에 연결되어있지 않아 결제를 진행할 수 없습니다.");
            return PurchaseProcessingResult.Pending;
        }
    
        if (ProductManager.IsRegisterProduct(args.purchasedProduct.definition.id) == false)
        {
            return PurchaseProcessingResult.Pending;
        }

#if UNITY_EDITOR
        ProductManager.PurchaseProductComplete(args.purchasedProduct.definition.id, true);
        return PurchaseProcessingResult.Pending;
#else
            if (ProductManager.IsRegisterProduct(args.purchasedProduct.definition.id) == false)
            {
    ...
    public static PurchaseState PurchaseProduct(string productId)
    {
        if (m_StoreController == null)
        {
            return PurchaseState.INITIALIZE_FAILED;
        }

        if (ProductManager.IsRegisterProduct(productId) == false)
        {
            return PurchaseState.NOT_REGISTERED;
        }
